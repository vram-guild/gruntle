...
if ! grep -q project_common.gradle @track_minecraft_version; then
  ...
  sed -e "" -i "s/^ext\.mod_version.*/ext\.mod_version = '$MOD_VERSION'/" 'project_common.gradle'
fi
...
sed -e "" -i "s/\"fabricloader\": \".*\"/\"fabricloader\": \">=$LOADER_VERSION\"/" fabric/src/main/resources/fabric.mod.json
sed -e "" -i "s/\"minecraft\": \".*\"/\"minecraft\": \">=$MC_SHORT_VERSION\"/" fabric/src/main/resources/fabric.mod.json
sed -e "" -i "s/\"architectury\": \".*\"/\"architectury\": \">=$ARCH_VERSION\"/" fabric/src/main/resources/fabric.mod.json
